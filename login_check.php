...
$return_to = '/of/links/'; // where are they going after successful login?
...
function get_user_options($user_id) {
	...
	$user_id = (int) $user_id * 1;
	$options = array();
	$get_option_row = $mysqli->query("SELECT * FROM user_options WHERE user_id=$user_id");
	if ($get_option_row->num_rows > 0) {
		while ($option_row = $get_option_row->fetch_assoc()) {
			$options[$option_row['optname']] = $option_row['optvalue'];
		}
	} else {
		return false;
	}
...
if (isset($_COOKIE[$session_cookie_name]) && trim($_COOKIE[$session_cookie_name]) != '') { // user has a session already?
	...
	if (strpos($user_session_complete_token, ':') === false) {
		...
		if ($check_for_session->num_rows == 1) {
			...
			$current_user['username'] = get_username($current_user['user_id']);
			$current_user['options'] = get_user_options($current_user['user_id']);
